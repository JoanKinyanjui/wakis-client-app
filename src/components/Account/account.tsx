...
function Account() {
  ...
  return (
   <CardWrapper customClass='mb-10'>
     ...
     <div className='w-full flex overflow-x-auto md:grid  md:w-[30%] lg:w-[20%] cursor-pointer no-scrollbar '>
      {accMenuList.map((item,index)=>(
        <div key={item.id} onClick={()=>selectActiveMenu(item.id)} className={`w-full mx-2 md:mx-0 ${activeContent === item.id ? 'bg-grey_102 ' :"bg-transparent"} rounded-[2px]  py-2 md:py-3 px-4 md:px-2 grid md:flex md:gap-2 items-start md:items-center`}>
