...
function InYourLocation() {
  return (
    <div className='w-full'>
        <CategoryHeader link='inYourLocation' icon='carbon:location-filled' headingLeft='IN YOUR LOCATION' rightIcon={true} headingRight='SEE ALL' />
      ...
      {Products.slice(0,6).map((item,index)=>(
        ...
        <ProductImage  item={item} id={item.id} price={item.price} beforePrice={item.beforePrice} imageUrl={item.imageUrl}  />
        </div>
      ))}
      </div>
    </div>
