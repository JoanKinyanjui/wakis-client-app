...
function page() {
  const [bagsProducts,setBagsProducts] = useState<ProductsProps[]>([]);
  useEffect(()=>{
    const checkBagsCategory =(item:any)=>{
     if(item.category.includes('Bags')){
      return item
     }
    }
    const results = Products.filter(checkBagsCategory);
    setBagsProducts(results);
  },[])
  ...
  return (
   ...
   {bagsProducts && bagsProducts.map((item,index)=>(
      <ProductImage 
      item={item}
      key={item.id}
      id={item.id}
